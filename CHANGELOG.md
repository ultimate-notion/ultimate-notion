...
- New: Support tab blocks via the new `Tabs` block, which groups content into labeled tabs. Each tab is a `Paragraph` whose text is the tab label, with an optional icon (now exposed via the new `Paragraph.icon` property) and children holding the tab content. Create tabs up front with `Tabs([...labels])` or add them later with `Tabs.add_tab()`, issue #188.
