...
# and `pip install ultimate-notion` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ultimate-notion", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ultimate-notion -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ultimate-notion", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ultimate-notion", "YOUR_CONSOLE_SCRIPT"]
