...
New pages are appended by default. Pass `position=uno.InsertPosition.START` or
`position=uno.InsertPosition.END` to place one explicitly, or use `after=some_child`
to create it after an existing direct child of its parent. An existing page can be
reparented with `page.move(new_parent)`; the new parent may be a page or data source.
...
To prepend or explicitly append blocks, pass `position=uno.InsertPosition.START` or
`position=uno.InsertPosition.END` to `append`. The `position` and `after` arguments
...
Enhanced Markdown can be inserted with `page.insert_markdown(markdown)`. It appends
by default; `position=uno.InsertPosition.START` prepends, while `after='start...end'`
uses Notion's ellipsis selection syntax to insert after matching content.
